        ...
        <div className="col-span-12 xl:col-span-8 flex flex-col">
          ...
          <section className="px-10 py-12 bg-white border-t border-border">
            <div className="flex items-center gap-4 mb-8">
              ...
              <h3 className="text-2xl font-extrabold text-foreground">Score Fusion Engine</h3>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-4">Model Voting Stack</p>
                <div className="bg-muted/20 rounded-2xl p-6 border border-border space-y-4">
                  {[
                    ...
                    <div key={m.name} className="flex items-center gap-3">
                      <div className="flex-1 h-8 rounded-xl bg-muted/30 overflow-hidden">
                        <div className={cn("h-full rounded-xl flex items-center px-3", m.color)} style={{ width: `${m.weight}%` }}>
                          <span className="text-[10px] font-bold text-white truncate">{m.name}</span>
                        </div>
                      </div>
                      <span className="text-sm font-mono font-bold text-muted-foreground w-12 text-right">{m.weight}%</span>
                    </div>
                  ))}
                </div>
              </div>
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-4">Decision Modifiers</p>
                <div className="bg-muted/20 rounded-2xl p-6 border border-border">
                  <div className="flex flex-wrap gap-3">
                    {[
                      ...
                      <span key={d.label} className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-card border border-border text-xs font-bold text-foreground shadow-sm">
          ...
          <section className="px-10 py-16 bg-white border-t border-border flex-1">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-4">
                ...
                <h3 className="text-2xl font-extrabold text-foreground">Model Leaderboard</h3>
              </div>
              <span className="text-[11px] font-black text-primary uppercase tracking-[0.2em]">View All</span>
            </div>
            <div className="bg-white rounded-[2.5rem] overflow-hidden border border-border shadow-sm">
              <table className="w-full text-left">
                <thead className="bg-muted/30">
                  <tr>
                    <th className="px-6 py-5 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Rank</th>
                    <th className="px-6 py-5 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Model Engine</th>
                    <th className="px-6 py-5 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Recall</th>
                    <th className="px-6 py-5 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Precision</th>
                    <th className="px-6 py-5 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border/50">
                  {MODELS.map((m) => (
                    <tr key={m.rank} className="hover:bg-muted/10 transition-colors">
                      <td className="px-6 py-6 font-mono font-extrabold text-foreground">{m.rank}</td>
                      <td className="px-6 py-6 font-bold text-foreground">{m.model}</td>
                      <td className="px-6 py-6 font-mono text-sm text-muted-foreground">{m.recall.toFixed(3)}</td>
                      <td className="px-6 py-6 font-mono text-sm text-muted-foreground">{m.precision.toFixed(3)}</td>
                      <td className="px-6 py-6">
                        <span className={cn("px-3 py-1 text-[10px] font-black rounded uppercase", m.statusCls)}>{m.status}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
