...
**Fast-path bypass condition:** `sigma_hit_count > 0 AND sigma_max_severity >= 'high'`. For these events Hunter immediately publishes the confirmed verdict to `hunter-results` (Kafka) without waiting for L1/L2, achieving < 15 ms output latency. Events that hit Sigma rules with `medium` or lower severity still enter the full pipeline.

**Critical — training data integrity:** Fast-path events run L1/L2 threads **asynchronously** after publishing the Kafka output. The 42-dim feature vector is only written to `hunter_training_data` once ALL thread results are available. This ensures CatBoost never trains on incomplete vectors with 26 zero-padded L1/L2 features.

```python
async def investigate(self, triage_msg: dict):
    sigma_hits = self.sigma_engine.evaluate(...)
    
    if self.sigma_engine.should_fast_path(sigma_hits):
        # 1. Publish verdict immediately (< 15ms)
        verdict = self._sigma_fast_verdict(sigma_hits, triage_msg)
        await self._publish(verdict)
        
        # 2. Run L1/L2 in background to complete the feature vector
        asyncio.create_task(self._complete_feature_vector_async(
            triage_msg, sigma_hits, verdict
        ))
        # Training data write happens inside _complete_feature_vector_async
        # NEVER write to hunter_training_data here with an incomplete vector
        return
    
    # Full pipeline: wait for all threads before publishing
    l1_results = await asyncio.gather(temporal, similarity, graph, spc)
    l2_results = await asyncio.gather(mitre, campaign)
    verdict = self.fusion.fuse(...)
    await self._publish(verdict)
    await self._write_training_data(triage_msg, feature_vector, verdict)
```
...
**Label assignment rules** — binary confidence threshold is WRONG. `probable_attack` (confidence=0.80) and `novel_investigation` (confidence=0.60) are not benign. Only write samples where the verdict is unambiguously positive OR unambiguously negative:

```python
# DEFINITE POSITIVES: High confidence AND attack-class finding type
DEFINITE_POSITIVE_TYPES = {
    'confirmed_attack', 'probable_attack', 'campaign',
    'evasion_technique', 'novel_anomaly', 'ioc_correlation'
}
# AMBIGUOUS TYPES: Never use as pseudo-negatives — could be undetected attacks
AMBIGUOUS_TYPES = {
    'novel_investigation', 'ml_alert', 'low_confidence'
}

def _write_training_data(
    self, alert_id: str, feature_vector: list[float],
    verdict: HunterVerdict, is_fast_path: bool = False
):
    """
    Write a labeled training sample. Only writes when the sample is
    unambiguously positive OR unambiguously negative.
    
    NEVER writes:
      - Fast-path samples before L1/L2 threads complete (incomplete vector)
      - Ambiguous findings (novel_investigation, ml_alert, low_confidence)
        that could be undetected attacks labeled as benign
    """
    if is_fast_path:
        # Feature vector is not yet complete — caller must await background threads
        raise RuntimeError("_write_training_data called with is_fast_path=True "
                           "before L1/L2 completed. Use _complete_feature_vector_async.")
    
    is_definite_pos = (
        verdict.confidence >= 0.85
        and verdict.finding_type in DEFINITE_POSITIVE_TYPES
    )
    is_definite_neg = (
        verdict.confidence < 0.30
        and verdict.severity == 'info'
        and verdict.finding_type not in DEFINITE_POSITIVE_TYPES
        and verdict.finding_type not in AMBIGUOUS_TYPES
    )
    
    if not is_definite_pos and not is_definite_neg:
        # Ambiguous — do not corrupt training set with uncertain labels
        logger.debug(
            f"Skipping training write for {verdict.finding_type} "
            f"(confidence={verdict.confidence:.2f}) — not unambiguously positive or negative"
        )
        return
    
    label = 1 if is_definite_pos else 0
    label_source = "pseudo_positive" if is_definite_pos else "pseudo_negative"
    ...
    logger.debug(
        f"Training sample written: {label_source}, "
        f"finding={verdict.finding_type}, confidence={verdict.confidence:.2f}"
    )
```

**Minimum negative ratio guard** (in `SelfSupervisedTrainer._train_and_save`):
```python
neg_count = sum(1 for r in dataset if r['label'] == 0)
if neg_count / len(dataset) < 0.10:
    logger.warning(
        f"Negative sample ratio {neg_count}/{len(dataset)} = "
        f"{neg_count/len(dataset):.1%} < 10%. "
        f"Model will be biased toward positives. Skipping retrain."
    )
    return
```
...
### Phase 4-0: Pre-Implementation Blockers (Day 0 — MUST complete before any code)

These two tables are hard dependencies for Phase 4B components. If they don't exist when the code is deployed, the Similarity Searcher and SPC Engine will fail immediately at runtime with no graceful fallback.

| Task | Description | Blocks |
|------|-------------|--------|
| Create `features_entity_freq` table in ClickHouse | **P0 BLOCKER** — missing despite being in `schema.sql`. Run migration script. Verify via `SELECT count() FROM clif_logs.features_entity_freq` | L1 Thread 4 (SPC) — `refresh_baselines()` query will throw on startup |
| Add `attack_embeddings` to `lancedb-service/app.py` `_ensure_tables()` | **P0 BLOCKER** — table never created. Add to LanceDB service alongside `log_embeddings`. Populate from `triage_scores WHERE action='escalate' AND adjusted_score > 0.8` (historical seed) | L1 Thread 2 (Similarity) — 3 of 7 features permanently neutral, evasion/novelty detection disabled |
| Verify both tables exist | `curl http://localhost:8100/tables` (LanceDB), `clickhouse-client --query "SHOW TABLES FROM clif_logs"` | Phase 4B |

> **Do not proceed to Phase 4A until both blockers are resolved and verified.**

### Phase 4A: Foundation + Sigma Fast Path (Days 1-5)

| Task | Description | Dependencies |
|------|-------------|-------------|
| Create `agents/hunter/` directory | Full structure per Section 20 | Phase 4-0 complete |
...
| SigmaEngine class | Fast-path evaluation with async L1/L2 background task | SQL compiler |
...
| Seed `attack_embeddings` with historical data | Populate from `triage_scores WHERE action=\'escalate\' AND adjusted_score > 0.8 AND ioc_match = 1` + high-severity `security_events` + `feedback_labels WHERE label=\'true_positive\'` | Phase 4-0 (table already exists) |
...
| Pseudo-negative label void + training writer label corruption | ~~High~~ **Eliminated** | ~~Critical~~ | V6 fix: Pseudo-negatives sourced from Hunter's own low-confidence investigations. Training writer now uses `is_definite_pos`/`is_definite_neg` guards — ambiguous finding types (`novel_investigation`, `ml_alert`, `low_confidence`) skip write entirely. `probable_attack` (conf=0.80) correctly labeled class 1. Min 10% negative ratio guard in trainer. |
| Fast-path feature vector corruption | ~~High~~ **Eliminated** | ~~Critical~~ | Fast-path publishes Kafka verdict immediately then runs L1/L2 via `asyncio.create_task()`. Training write deferred until async task completes. `_write_training_data(is_fast_path=True)` raises `RuntimeError` before threads finish — prevents 26-zero feature vector from entering training set. |
...
| `attack_embeddings` table missing at Similarity Searcher deployment | ~~Certain~~ **Eliminated** | ~~High~~ | Moved to **Phase 4-0 (Day 0 Blocker)**. Must exist before Phase 4A. Verified via `curl http://localhost:8100/tables`. Phase 4C seeds it with historical data only — table creation blocked to Phase 4-0. |
