-- =============================================================================
-- CLIF ClickHouse Schema — Stage 3 Storage Layer (LOCAL / Single-Node)
-- =============================================================================
-- Combined from schema.sql + hunter_schema.sql with ON CLUSTER removed.
...
CREATE TABLE IF NOT EXISTS clif_logs.events_per_minute
(
...
CREATE MATERIALIZED VIEW IF NOT EXISTS clif_logs.events_per_minute_mv
...
CREATE MATERIALIZED VIEW IF NOT EXISTS clif_logs.security_severity_hourly_mv
...
CREATE MATERIALIZED VIEW IF NOT EXISTS clif_logs.events_per_minute_security_mv
TO clif_logs.events_per_minute
AS
SELECT
    toStartOfMinute(timestamp) AS minute,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS clif_logs.events_per_minute_process_mv
TO clif_logs.events_per_minute
AS
SELECT
    toStartOfMinute(timestamp) AS minute,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS clif_logs.events_per_minute_network_mv
TO clif_logs.events_per_minute
AS
SELECT
    toStartOfMinute(timestamp) AS minute,
...
CREATE TABLE IF NOT EXISTS clif_logs.events_per_10s
(
...
CREATE MATERIALIZED VIEW IF NOT EXISTS clif_logs.events_per_10s_raw_mv
...
CREATE MATERIALIZED VIEW IF NOT EXISTS clif_logs.events_per_10s_security_mv
TO clif_logs.events_per_10s
AS
SELECT
    toStartOfInterval(now(), INTERVAL 10 SECOND) AS ts,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS clif_logs.events_per_10s_process_mv
TO clif_logs.events_per_10s
AS
SELECT
    toStartOfInterval(now(), INTERVAL 10 SECOND) AS ts,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS clif_logs.events_per_10s_network_mv
TO clif_logs.events_per_10s
AS
SELECT
    toStartOfInterval(now(), INTERVAL 10 SECOND) AS ts,
...
-- ─────────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS clif_logs.pipeline_metrics
(
...
CREATE TABLE IF NOT EXISTS clif_logs.allowlist
(
...
CREATE TABLE IF NOT EXISTS clif_logs.ioc_cache
(
...
CREATE TABLE IF NOT EXISTS clif_logs.source_thresholds
(
...
CREATE TABLE IF NOT EXISTS clif_logs.asset_criticality
(
...
(
    ...
    evidence_json     String        DEFAULT ''                    CODEC(ZSTD(3)),
    -- XAI: structured decision trail (decision path, calibration, per-check details)
    explanation_json  String        DEFAULT ''                    CODEC(ZSTD(3)),
...
CREATE TABLE IF NOT EXISTS clif_logs.dead_letter_events
(
...
CREATE TABLE IF NOT EXISTS clif_logs.mitre_mapping_rules
(
...
CREATE TABLE IF NOT EXISTS clif_logs.features_entity_freq
(
...
CREATE MATERIALIZED VIEW IF NOT EXISTS clif_logs.features_entity_freq_security_mv
...
CREATE MATERIALIZED VIEW IF NOT EXISTS clif_logs.features_entity_freq_network_mv
TO clif_logs.features_entity_freq
AS
SELECT
    toStartOfMinute(timestamp) AS window,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS clif_logs.features_entity_freq_process_mv
TO clif_logs.features_entity_freq
AS
SELECT
    toStartOfMinute(timestamp) AS window,
...
CREATE TABLE IF NOT EXISTS clif_logs.features_template_rarity
(
...
CREATE TABLE IF NOT EXISTS clif_logs.features_entity_baseline
(
    user_id          String         CODEC(ZSTD(1)),
    hostname         String         CODEC(ZSTD(1)),
...
CREATE MATERIALIZED VIEW IF NOT EXISTS clif_logs.features_entity_baseline_mv
...
CREATE TABLE IF NOT EXISTS clif_logs.triage_score_rollup
(
    hour            DateTime       CODEC(Delta, ZSTD(1)),
    source_type     LowCardinality(String) CODEC(ZSTD(1)),
    action          Enum8('discard'=0, 'monitor'=1, 'escalate'=2) CODEC(ZSTD(1)),
...
CREATE MATERIALIZED VIEW IF NOT EXISTS clif_logs.triage_score_rollup_mv
...
CREATE VIEW IF NOT EXISTS clif_logs.features_mv_staleness
AS
SELECT
    table_name,
    max_ts,
...
CREATE TABLE IF NOT EXISTS clif_logs.arf_replay_buffer
(
...
-- =============================================================================
-- HUNTER AGENT SCHEMA ADDITIONS (from hunter_schema.sql)
-- =============================================================================


-- ─────────────────────────────────────────────────────────────────────────────
-- 25. entity_baselines — SPC per-entity behavioral baselines
-- ─────────────────────────────────────────────────────────────────────────────
--    Updated every 60 seconds by SPCEngine.refresh_baselines().
--    Key: (hostname, metric_name). Supports 22 hosts × 6 metrics = 132 rows.

CREATE TABLE IF NOT EXISTS clif_logs.entity_baselines
(
    hostname          String                                      CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS clif_logs.sigma_rule_hits_dist
AS clif_logs.sigma_rule_hits
ENGINE = Distributed('clif_cluster', clif_logs, sigma_rule_hits, rand());

-- ─────────────────────────────────────────────────────────────────────────────
-- 27. hunter_training_data — Self-supervised CatBoost training samples
-- ─────────────────────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS clif_logs.hunter_training_data
(
    alert_id          String                                     CODEC(ZSTD(1)),
...
CREATE TABLE IF NOT EXISTS clif_logs.hunter_model_health_dist
